...
for (var _i = 0, files_1 = files; _i < files_1.length; _i++) {
    ...
    if (file.slice(-5) === ".room") {
        ...
        console.log("Loaded ".concat(room_posts.length, " posts on room ").concat(room_name, "."));
...
function save_post(post_room, post_user, post_data) {
    ...
    if (Watchlist[valid_post_room]) {
        ...
        for (var _i = 0, _a = Watchlist[valid_post_room]; _i < _a.length; _i++) {
            var WebSocket = _a[_i];
...
// TCP API
// =======
var WebSockets = new WebSocket.Server({ port: port });
...
WebSockets.on("connection", function connection(WebSocket) {
    ...
    WebSocket.on("close", function () {
        for (var room_name in Watchlist) {
            Watchlist[room_name] = Watchlist[room_name].filter(function (watcher) { return watcher !== WebSocket; });
        }
        console.log("[" + (--Connected) + " connected]");
    });
});
